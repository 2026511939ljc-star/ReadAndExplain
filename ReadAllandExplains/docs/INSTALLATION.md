# ReadAllandExplains 4.8.1 安装指南

ReadAllandExplains 4.8.1 面向 Unreal Engine 5.7 / Windows 64 位。Release ZIP 同时包含 UE 编辑器插件、CodeBuddy Skill、只读 MCP 与诊断命令。
...
1. 关闭 Unreal Editor。
2. 从 GitHub Release 下载 `ReadAllandExplains_4.8.1_UE5.7_Win64.zip`。
...
3. 删除旧插件目录，避免旧文件残留。
4. 解压 4.8.1 ZIP 到相同位置。
5. 重启 UE，并在 CodeBuddy 中执行 `/reload-plugins`。
6. 再次运行诊断命令并生成一个新的 Context Pack。

旧 Pack 仍可读取。4.8.1 会改变受影响 Blueprint Pin 的稳定 ID 表示，因此不要跨 4.8.0 与 4.8.1 Pack 复用 Pin ID 缓存；建议升级后重新生成关键资产快照。
...
ReadAllandExplains 4.8.1 targets Unreal Engine 5.7 on Windows 64-bit. The Release ZIP includes the UE editor plugin, CodeBuddy Skill, read-only MCP server, and diagnostics command.
...
2. Download `ReadAllandExplains_4.8.1_UE5.7_Win64.zip` from GitHub Releases.
...
Close UE and CodeBuddy, back up and remove the old plugin directory, extract 4.8.1 to the same location, restart UE, reload CodeBuddy plugins, run diagnostics, and generate a new Pack. Existing Packs remain readable, but affected Blueprint Pin IDs use a new stable representation in 4.8.1. Do not reuse Pin ID caches across 4.8.0 and 4.8.1 Packs; regenerate important snapshots after upgrading.
