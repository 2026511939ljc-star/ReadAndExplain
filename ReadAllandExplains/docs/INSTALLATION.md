# ReadAllandExplains 4.7.0 安装指南

ReadAllandExplains 4.7.0 面向 Unreal Engine 5.7 / Windows 64 位。Release ZIP 同时包含 UE 编辑器插件、CodeBuddy Skill、只读 MCP 与诊断命令。
...
1. 关闭 Unreal Editor。
2. 从 GitHub Release 下载 `ReadAllandExplains_4.7.0_UE5.7_Win64.zip`。
...
ReadAllandExplains 4.7.0 targets Unreal Engine 5.7 on Windows 64-bit. The Release ZIP includes the UE editor plugin, CodeBuddy Skill, read-only MCP server, and diagnostics command.
...
2. Download `ReadAllandExplains_4.7.0_UE5.7_Win64.zip` from GitHub Releases.
...
Close UE and CodeBuddy, back up and remove the old plugin directory, extract 4.7.0 to the same location, restart UE, reload CodeBuddy plugins, run diagnostics, and generate a new Pack. Existing 4.6 Packs remain readable, but regenerating important snapshots is recommended.
