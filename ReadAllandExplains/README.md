...
[![Version](https://img.shields.io/badge/version-4.8.0--preview.2-blue)](https://github.com/2026511939ljc-star/ReadAndExplain/releases)
...
> `v4.8.0-preview.2` 新增渐进式证据查询：`get_asset_outline` 先给规模再决定读不读、`locate_graph_target` 确定性定位并支持省略 `query` 枚举发现、`get_graph_subgraph` 按跳数/节点/字符预算取最小闭包并诚实报告截断边界、稳定章节按 `section_id` 精确读取。写操作 `request_targeted_snapshot` 的 `pack_path` 改为必填，确保补拍始终绑定明确基线。全部新能力位于 MCP 层，UE 插件与导出结构不变，现有 4.7 Pack 无需重新导出。
>
