...
[![Version](https://img.shields.io/badge/version-4.8.1-blue)](https://github.com/2026511939ljc-star/ReadAndExplain/releases)
...
> `v4.8.1` 修复重复导出与跨 UE 进程的两类确定性漂移：Texture 可读文档改用导入源尺寸，Blueprint Pin 改用持久 GUID 或结构身份。两个独立 UE 5.7 进程生成的四份 Waterfall Pack 均为 39/39，规范化 Golden 82/82 全绿；受影响的 Blueprint `pin_id` 会发生一次性格式迁移，升级后建议重导关键 Pack。
>
...
> `v4.8.1` fixes two determinism issues across repeated exports and fresh UE processes: Texture readable documents now use imported source dimensions, and Blueprint Pins use persistent GUIDs or structural identities. Four Waterfall Packs from two independent UE 5.7 processes completed at 39/39 and passed normalized Golden comparison at 82/82. Affected Blueprint `pin_id` values undergo a one-time representation migration, so regenerate important Packs after upgrading.
>
