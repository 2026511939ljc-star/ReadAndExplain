...
namespace
{
	...
	static FString EmitValue(FEmitContext& Ctx, const UMaterialExpression* Expr)
	{
		...
		else if (const UMaterialExpressionCustom* Custom = Cast<UMaterialExpressionCustom>(Expr))
		{
			...
			for (int32 i = 0; i < Custom->Inputs.Num(); ++i)
			{
				...
				FString InputName = CInput.InputName;
			...
			FString OutType = Custom->OutputType;
			if (OutType.IsEmpty()) OutType = TEXT("Float3");
		...
		else
		{
			...
			const int32 NumInputs = CountExpressionInputs(MutableExpr);
	...
	static FString GetMaterialPropertyLabel(const EMaterialProperty Property)
	{
		if (const UEnum* Enum = StaticEnum<EMaterialProperty>())
		{
			const FText DisplayName = Enum->GetDisplayNameTextByValue(static_cast<int64>(Property));
			const FString EnumName = Enum->GetNameStringByValue(static_cast<int64>(Property));
			return DisplayName.IsEmpty() ? EnumName : DisplayName.ToString().TrimStartAndEnd();
		}
		return FString::Printf(TEXT("MaterialProperty_%d"), static_cast<int32>(Property));
	}

	static void EmitAllMaterialPropertyBlocks(UMaterial* Mat, FEmitContext& Ctx, FString& OutText)
	{
		if (!Mat) return;
		...
		for (int32 PropertyIndex = 0; PropertyIndex < static_cast<int32>(MP_MAX); ++PropertyIndex)
		{
			...
			const FExpressionInput* Input = Mat->GetExpressionInputForProperty(Property);
			if (!Input || !Input->Expression) continue;
			const FString Label = GetMaterialPropertyLabel(Property);
			EmitMaterialPropertyBlock(Mat, Property, *Label, Ctx, OutText);
			++ConnectedCount;
		}

		if (ConnectedCount == 0)
		{
			OutText += TEXT("- (no connected material root properties)\n\n");
		}
	}

	static FString DescribeParameterInfo(const FMaterialParameterInfo& Info)
	{
		return FString::Printf(
			TEXT("%s [Association=%s, Index=%d]"),
			*Info.Name.ToString(),
			*EnumToString(Info.Association),
			Info.Index);
	}

	static void EmitStaticParameters(UMaterialInterface* MaterialInterface, FString& OutText)
	{
		if (!MaterialInterface) return;

		OutText += TEXT("### Static Switches\n\n");
		OutText += TEXT("| Parameter | FinalValue | ExpressionGuid |\n");
		OutText += TEXT("|-----------|------------|----------------|\n");
		TArray<FMaterialParameterInfo> SwitchInfos;
		TArray<FGuid> SwitchIds;
		MaterialInterface->GetAllStaticSwitchParameterInfo(SwitchInfos, SwitchIds);
		for (const FMaterialParameterInfo& Info : SwitchInfos)
		{
			bool bValue = false;
			FGuid ExpressionGuid;
			const bool bFound = MaterialInterface->GetStaticSwitchParameterValue(
				FHashedMaterialParameterInfo(Info), bValue, ExpressionGuid);
			OutText += FString::Printf(
				TEXT("| %s | %s | %s |\n"),
				*SanitizeForMarkdownCell(DescribeParameterInfo(Info)),
				bFound ? (bValue ? TEXT("true") : TEXT("false")) : TEXT("<unset>"),
				*ExpressionGuid.ToString(EGuidFormats::DigitsWithHyphens));
		}
		if (SwitchInfos.Num() == 0) OutText += TEXT("| (none) | | |\n");

		OutText += TEXT("\n### Static Component Masks\n\n");
		OutText += TEXT("| Parameter | FinalValue RGBA | ExpressionGuid |\n");
		OutText += TEXT("|-----------|-----------------|----------------|\n");
		TArray<FMaterialParameterInfo> MaskInfos;
		TArray<FGuid> MaskIds;
		MaterialInterface->GetAllStaticComponentMaskParameterInfo(MaskInfos, MaskIds);
		for (const FMaterialParameterInfo& Info : MaskInfos)
		{
			bool R = false, G = false, B = false, A = false;
			FGuid ExpressionGuid;
			const bool bFound = MaterialInterface->GetStaticComponentMaskParameterValue(
				FHashedMaterialParameterInfo(Info), R, G, B, A, ExpressionGuid);
			OutText += FString::Printf(
				TEXT("| %s | %s | %s |\n"),
				*SanitizeForMarkdownCell(DescribeParameterInfo(Info)),
				bFound ? *FString::Printf(TEXT("(%d,%d,%d,%d)"), R, G, B, A) : TEXT("<unset>"),
				*ExpressionGuid.ToString(EGuidFormats::DigitsWithHyphens));
		}
		if (MaskInfos.Num() == 0) OutText += TEXT("| (none) | | |\n");
		OutText += TEXT("\n");

		if (const UMaterialInstance* Instance = Cast<UMaterialInstance>(MaterialInterface))
		{
			const FStaticParameterSetEditorOnlyData& StaticSet = Instance->GetEditorOnlyStaticParameters();
			OutText += FAssetTextSnapshot::ExportStructProperties(
				FStaticParameterSetEditorOnlyData::StaticStruct(),
				&StaticSet,
				const_cast<UMaterialInstance*>(Instance),
				TEXT("Raw Static Parameter Set (includes layer stack data)"));
		}
	}

	static UMaterialGraph* BuildMaterialGraphSnapshot(UMaterial* Material)
	{
		...
		UMaterialGraph* Graph = CastChecked<UMaterialGraph>(FBlueprintEditorUtils::CreateNewGraph(
			GetTransientPackage(), NAME_None, UMaterialGraph::StaticClass(), UMaterialGraphSchema::StaticClass()));
		Graph->Material = Material;
		Graph->MaterialFunction = nullptr;
		Graph->OriginalMaterialFullName = Material->GetPathName();
		Graph->RebuildGraph();
		return Graph;
	}

	static void EmitExpressionSnapshots(
		TConstArrayView<TObjectPtr<UMaterialExpression>> Expressions,
		TConstArrayView<TObjectPtr<UMaterialExpressionComment>> Comments,
		FString& OutText)
	{
		OutText += TEXT("## Complete Expression Object Snapshots\n\n");
		OutText += TEXT("These tables retain persistent reflected fields for every expression, including custom HLSL code, function references, parameter metadata, texture settings and node-specific options.\n\n");
		for (int32 Index = 0; Index < Expressions.Num(); ++Index)
		{
			if (const UMaterialExpression* Expression = Expressions[Index])
			{
				OutText += FAssetTextSnapshot::ExportObjectProperties(
					Expression,
					FString::Printf(TEXT("Expression %d: %s"), Index, *Expression->GetName()));
			}
		}
		for (int32 Index = 0; Index < Comments.Num(); ++Index)
		{
			if (const UMaterialExpressionComment* Comment = Comments[Index])
			{
				OutText += FAssetTextSnapshot::ExportObjectProperties(
					Comment,
					FString::Printf(TEXT("Comment %d: %s"), Index, *Comment->GetName()));
			}
		}
	}

	static void EmitFunctionInterfaces(const UMaterialFunctionInterface* Function, FString& OutText)
	{
		if (!Function) return;
		OutText += TEXT("## Function Interface\n\n");
		OutText += TEXT("| Kind | Name | Type / Root | Sort | Guid | Default / Source |\n");
		OutText += TEXT("|------|------|-------------|-----:|------|------------------|\n");
		for (const UMaterialExpression* Expression : Function->GetExpressions())
		{
			...
			{
				const FString PreviewSource = Input->Preview.Expression
					? Input->Preview.Expression->GetName()
					: FString::Printf(TEXT("(%g,%g,%g,%g)"), Input->PreviewValue.X, Input->PreviewValue.Y, Input->PreviewValue.Z, Input->PreviewValue.W);
				OutText += FString::Printf(TEXT("| Input | %s | %s | %d | %s | %s |\n"),
					*SanitizeForMarkdownCell(Input->InputName.ToString()),
					*EnumToString(Input->InputType),
					Input->SortPriority,
					*Input->Id.ToString(EGuidFormats::DigitsWithHyphens),
					*SanitizeForMarkdownCell(PreviewSource));
			}
			else if (const UMaterialExpressionFunctionOutput* Output = Cast<UMaterialExpressionFunctionOutput>(Expression))
			{
				const FString Source = Output->A.Expression
					? FString::Printf(TEXT("%s.Output[%d]"), *Output->A.Expression->GetName(), Output->A.OutputIndex)
					: TEXT("<unconnected>");
				OutText += FString::Printf(TEXT("| Output | %s | Result | %d | %s | %s |\n"),
					*SanitizeForMarkdownCell(Output->OutputName.ToString()),
					Output->SortPriority,
					*Output->Id.ToString(EGuidFormats::DigitsWithHyphens),
					*SanitizeForMarkdownCell(Source));
			}
		}
		OutText += TEXT("\n");
	}

	static void EmitFunctionGraphIndex(const UMaterialFunctionInterface* Function, FString& OutText)
	{
		if (!Function) return;
		...
		for (int32 Index = 0; Index < Expressions.Num(); ++Index)
		{
			if (const UMaterialExpression* Expression = Expressions[Index])
			{
				IdByExpression.Add(Expression, Index);
			}
		}
		...
		for (int32 Index = 0; Index < Expressions.Num(); ++Index)
		{
			...
			const FString Desc = Expression->Desc.IsEmpty() ? Expression->GetName() : Expression->Desc;
			OutText += FString::Printf(TEXT("| %d | %s | %s |\n"),
				Index,
				*SanitizeForMarkdownCell(Expression->GetClass()->GetName()),
				*SanitizeForMarkdownCell(Desc));
		}

		OutText += TEXT("\n### Links\n\n");
		OutText += TEXT("| ConsumerId | Input | ProducerId | ProducerOutput | Mask |\n");
		OutText += TEXT("|-----------:|-------|-----------:|---------------:|------|\n");
		for (int32 Index = 0; Index < Expressions.Num(); ++Index)
		{
			UMaterialExpression* Expression = Expressions[Index];
			if (!Expression) continue;
			const int32 NumInputs = CountExpressionInputs(Expression);
			for (int32 InputIndex = 0; InputIndex < NumInputs; ++InputIndex)
			{
				const FExpressionInput* Input = Expression->GetInput(InputIndex);
				if (!Input || !Input->Expression) continue;
				const int32* ProducerId = IdByExpression.Find(Input->Expression);
				const FString Mask = FString::Printf(TEXT("%d%d%d%d"), Input->MaskR, Input->MaskG, Input->MaskB, Input->MaskA);
				OutText += FString::Printf(TEXT("| %d | %s | %d | %d | %s |\n"),
					Index,
					*SanitizeForMarkdownCell(Expression->GetInputName(InputIndex).ToString()),
...
FString FMaterialToTextExporter::ExportMaterialToText(UMaterialInterface* MaterialInterface)
{
	...
	Out += TEXT("## Pseudo HLSL (Code-First)\n");
	{
		...
		else
		{
			...
			EmitAllMaterialPropertyBlocks(BaseMaterial, Ctx, Out);

			Out += TEXT("- 注：此处为“材质图语义级伪 HLSL”，非 UE 最终编译产物；全部材质根属性均会遍历，未覆盖节点会以 `UnknownExpr(\"Class\", \"Title\", ...)` 形式保留。\n");
	...
	Out += TEXT("## Graph Index\n");
	{
		...
		else
		{
			...
			for (int32 i = 0; i < AllExprs.Num(); ++i)
			{
				...
				const int32 NumInputs = CountExpressionInputs(Expr);
	...
	if (Mode == EReadAllExportMode::Reconstruction)
	{
		...
		if (BaseMaterial)
		{
			EmitExpressionSnapshots(BaseMaterial->GetExpressions(), BaseMaterial->GetEditorComments(), Out);
			if (UMaterialGraph* Graph = BuildMaterialGraphSnapshot(BaseMaterial))
			{
				Out += FAssetTextSnapshot::ExportGraph(Graph, TEXT("Reconstructable Material Graph Snapshot"));
			}
		}
	}

	Out += TEXT("\n");
	EmitStaticParameters(MaterialInterface, Out);
	Out += FAssetTextSnapshot::ExportObjectProperties(MaterialInterface, TEXT("Material Interface Persistent Properties"));
	if (UMaterial* BaseMaterial = MaterialInterface->GetMaterial())
	{
		Out += FAssetTextSnapshot::ExportObjectProperties(BaseMaterial, TEXT("Base Material Persistent Properties"));
	}
...
FString FMaterialToTextExporter::ExportMaterialFunctionToText(UMaterialFunctionInterface* MaterialFunction)
{
	...
	EmitFunctionInterfaces(MaterialFunction, Out);
	Out += FAssetTextSnapshot::ExportObjectProperties(MaterialFunction, TEXT("Material Function Persistent Properties"));
	Out += TEXT("\n");
